...
// When form submitted, update database
if (isset($_POST['save'])) {
    $ISBN = $_POST['isbn'];
    $title = $_POST['title'];
    ...
    $id = $_GET['id'];
    $sql = "UPDATE inventory SET isbn='$ISBN', title='$title', classify='$classify', author='$author', publication_date='$publication_date' WHERE id = $id";
    $result = $mysqli->query($sql);
...
<body>
    <div class="wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <h1 class="border-bottom mt-3 mb-2 pb-3">Update Book</h1>

                    <form method="POST" action="#">
                        <div class="form-group">
                            <label class="fw-bold fs-5" for="isbn">ISBN</label>
                            <input class="form-control" id="isbn" name="isbn" value="<?php echo $book['isbn']; ?>" />
                            <p class="error" id="error-isbn"></p>
                        </div>

                        <div class="form-group">
                            <label class="fw-bold fs-5" for="title">Title</label>
                            <input class="form-control" id="title" name="title" value="<?php echo $book['title'] ?>" />
                            <p class="error" id="error-title"></p>
                        </div>

                        <div class="form-group">
                            <label class="fw-bold fs-5" for="classify">Classify</label>
                            <input class="form-control" id="classify" name="classify" value="<?php echo $book['classify'] ?>" />
                            <p class="error" id="error-classify"></p>
                        </div>

                        <div class="form-group">
                            <label class="fw-bold fs-5" for="author">Author</label>
                            <input class="form-control" id="author" name="author" value="<?php echo $book['author']; ?>" />
                            <p class="error" id="error-author"></p>
                        </div>

                        <div class="form-group">
                            <label class="fw-bold fs-5" for="publication_date">Publication date</label>
                            <input class="form-control" id="publication_date" name="publication_date" type="date" value="<?php echo $book['publication_date']; ?>" />
                        </div>
